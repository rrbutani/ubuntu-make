...
class TestRequirementsHandler(DpkgAptSetup):
    ...
    def wait_for_callback(self, mock_function_to_be_called, timeout=10):
        ...
        while not mock_function_to_be_called.called:
            if time() > timeout_time:
                raise (
                    BaseException(
                        "Function not called within {} seconds".format(timeout)
                    )
                )
    ...
    def test_install_progress(self):
        ...
        self.handler.install_bucket(
            ["testpackage"], progress_callback, self.done_callback
        )
        self.wait_for_callback(self.done_callback)

        downloading_msg = self.count_number_progress_call(
            progress_callback.call_args_list, RequirementsHandler.STATUS_DOWNLOADING
        )
        installing_msg = self.count_number_progress_call(
            progress_callback.call_args_list, RequirementsHandler.STATUS_INSTALLING
        )
        ...
        self.assertIn(
            progress_callback.call_args_list[0][0][0],
            [
        ...
        for call_item in progress_callback.call_args_list:
            if call_item[0][0] == {"step": 1, "percentage": 0.0}:
                ...
                break
        self.assertTrue(
            callfound, "We expect to have one install step at 0% call in the list"
        )

    def test_install_multiple_packages(self):
        """Install multiple packages in one shot"""
        self.handler.install_bucket(
            ["testpackage", "testpackage0"], lambda x: "", self.done_callback
        )
        self.wait_for_callback(self.done_callback)

        self.assertEqual(
            self.done_callback.call_args[0][0].bucket, ["testpackage", "testpackage0"]
        )
        self.assertIsNone(self.done_callback.call_args[0][0].error)
        self.assertTrue(
            self.handler.is_bucket_installed(["testpackage", "testpackage0"])
        )
    ...
    def test_install_multiple_packages_progress(self):
        ...
        progress_callback = Mock()
        self.handler.install_bucket(
            ["testpackage", "testpackage0"], progress_callback, self.done_callback
        )
        self.wait_for_callback(self.done_callback)
    ...
    def test_install_pending_order(self):
        ...
        self.handler.install_bucket(
            ["testpackage0"], progress_callback0, done_callback0
        )
        self.wait_for_callback(done_callback)
        self.wait_for_callback(done_callback0)

        self.assertEqual(
            self.done_callback.call_args_list,
            [
                call(
                    RequirementsHandler.RequirementsResult(
                        bucket=["testpackage"], error=None
                    )
                ),
                call(
                    RequirementsHandler.RequirementsResult(
                        bucket=["testpackage0"], error=None
                    )
                ),
            ],
        )
    ...
    def test_install_pending_callback_not_mixed(self):
        ...
        self.assertTrue(
            progress_callback.call_count < global_progress_callback.call_count
        )
        self.assertTrue(
            progress_callback0.call_count < global_progress_callback.call_count
        )
    ...
    def test_install_twice(self):
        ...
        self.handler.install_bucket(["testpackage"], progress_callback, done_callback)
        self.handler.install_bucket(
            ["testpackage"], progress_second_callback, self.done_callback
        )
        self.wait_for_callback(done_callback)
        self.wait_for_callback(self.done_callback)
    ...
    def test_deps(self):
        ...
        self.assertTrue(
            self.handler.is_bucket_installed(["testpackage1", "testpackage"])
        )

    def test_fail(self):
        """An error is caught when asking for the impossible (installing 2 packages in conflicts)"""
        self.handler.install_bucket(
            ["testpackage", "testpackage2"], lambda x: "", self.done_callback
        )
        self.wait_for_callback(self.done_callback)

        self.assertIsNotNone(self.done_callback.call_args[0][0].error)
        both_package_installed = self.handler.is_bucket_installed(
            ["testpackage"]
        ) and self.handler.is_bucket_installed(["testpackage2"])
    ...
    def test_is_installed_bucket_installed(self):
        ...
        self.handler.install_bucket(
            ["testpackage", "testpackage1"], lambda x: "", self.done_callback
        )
        self.wait_for_callback(self.done_callback)
        self.assertTrue(
            self.handler.is_bucket_installed(["testpackage", "testpackage1"])
        )
    ...
    def test_is_bucket_installed_multi_arch_current_arch(self):
        ...
        self.assertTrue(
            self.handler.is_bucket_installed(
                ["testpackage:{}".format(tools.get_current_arch())]
            )
        )
    ...
    def test_is_bucket_installed_with_unavailable_multiarch_package(self):
        ...
        self.assertFalse(
            self.handler.is_bucket_installed(["testpackagedoesntexist:foo"])
        )
    ...
    def test_is_bucket_uptodate_bucket_uptodate(self):
        ...
        self.wait_for_callback(self.done_callback)
        self.assertTrue(
            self.handler.is_bucket_uptodate(["testpackage", "testpackage1"])
        )
    ...
    def test_is_bucket_uptodate_with_unavailable_multiarch_package(self):
        ...
        self.assertFalse(
            self.handler.is_bucket_uptodate(["testpackagedoesntexist:foo"])
        )
    ...
    def test_is_bucket_uptodate_with_foreign_archs(self):
        ...
        self.handler.cache.open()  # reopen the cache with the new added architecture
        self.handler.install_bucket(
            ["testpackagefoo:foo"], lambda x: "", self.done_callback
        )
        self.wait_for_callback(self.done_callback)
    ...
    def test_is_bucket_uptodate_with_possible_upgrade(self):
        ...
        shutil.copy(
            os.path.join(self.apt_status_dir, "testpackage_installed_dpkg_status"),
    ...
    def test_is_bucket_available(self):
        ...
        self.assertTrue(
            self.handler.is_bucket_available(["testpackage", "testpackage1"])
        )
    ...
    def test_unavailable_bucket(self):
        ...
        self.assertFalse(
            self.handler.is_bucket_available(["testpackage42", "testpackage404"])
        )
    ...
    def test_is_bucket_available_foreign_archs(self):
        ...
        self.assertTrue(
            self.handler.is_bucket_available(["testpackagefoo:foo", "testpackage1"])
        )
    ...
    def test_is_bucket_unavailable_with_foreign_archs(self):
        ...
        self.assertFalse(
            self.handler.is_bucket_available(["testpackagebar:foo", "testpackage1"])
        )
    ...
    def test_bucket_unavailable_foreign_archs_no_added_another_package_not_available(
        self,
    ):
        """Bucket is set as unavailable when foreign arch not added, but another package on current arch is
         unavailable"""
        self.assertFalse(
            self.handler.is_bucket_available(["testpackagefoo:foo", "testpackage123"])
        )
    ...
    def test_apt_cache_not_ready(self):
        ...
        with patch.object(
            self.handler.cache, "open", side_effect=cache_call
        ) as openaptcache_mock:
            self.handler.install_bucket(
                ["testpackage"], lambda x: "", self.done_callback
            )
            self.wait_for_callback(self.done_callback)
