...
class BaseInstallerInContainer(ContainerTests, test_baseinstaller.BaseInstallerTests):
    ...
    def setUp(self):
        ...
        self.additional_local_frameworks = [
            os.path.join("tests", "data", "testframeworks", "baseinstallerfake.py")
        ]
        self.umake_download_page = os.path.join(
            get_data_dir(),
            "server-content",
            "github.com",
            "ubuntu",
            ...
            with swap_file_and_restore(self.umake_download_page) as content:
                with open(self.umake_download_page, "w") as newfile:
                    ...
                    version = subprocess.check_output(
                        self.command_as_list([UMAKE, "--version"]),
                        stderr=subprocess.STDOUT,
                    ).decode("utf-8")
                    newfile.write(
                        content.replace("LATESTRELEASE", version.strip().split("+")[0])
                    )
                self.child = spawn_process(
                    self.command("{} base base-framework".format(UMAKE))
                )
                ...
                # we have nothing installed
                self.assertFalse(
                    self.launcher_exists_and_is_pinned(self.desktop_filename)
                )
