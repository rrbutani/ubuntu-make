...
class ContainerTests(LoggedTestCase):
    ...
    def setUp(self):
        ...
        self.install_base_path = os.path.expanduser(
            "/home/{}/{}".format(self.DOCKER_USER, INSTALL_DIR)
        )
        self.binary_dir = self.binary_dir.replace(
            os.environ["HOME"], "/home/{}".format(self.DOCKER_USER)
        )
        ...
        runner_cmd = "mkdir -p {}; ln -s {}/ {};".format(
            os.path.dirname(get_root_dir()),
        ...
        for additional_framework in self.additional_local_frameworks:
            runner_cmd += "cp {} {};".format(
                os.path.join(self.UMAKE_TOOLS_IN_CONTAINER, additional_framework),
                local_framework_dir,
            )
        ...
        if not BRANCH_TESTS:
            ...
            runner_cmd += (
                ...
                "\n  main()'>{bin_umake}; chmod +x {bin_umake};".format(
                    bin_umake=bin_umake
                )
            )
        ...
        for port, hostnames in self.hosts.items():
            ...
            if ftp_redir:
                runner_cmd += "/usr/bin/twistd ftp -p 21 -r {};".format(
                    os.path.join(get_data_dir(), "server-content", hostnames[0])
                )
        ...
        if not BRANCH_TESTS:
            command.extend(
                [
                    "-v",
                    "{system_umake}:{system_umake}".format(
                        system_umake=SYSTEM_UMAKE_DIR
                    ),
                ]
            )
        ...
        command.extend(
            [
                "-d",
                "-v",
                "{}:{}".format(self.umake_path, self.UMAKE_TOOLS_IN_CONTAINER),
                "--dns=8.8.8.8",
                "--dns=8.8.4.4",  # suppress local DNS warning
                self.image_name,
                "sh",
                "-c",
                runner_cmd,
        ...
        self.conf_path = os.path.expanduser(
            "/home/{}/.config/umake".format(self.DOCKER_USER)
        )
        sleep(5)  # let the container and service starts

    def tearDown(self):
        subprocess.check_call(
            [get_docker_path(), "stop", "-t", "0", self.container_id],
            stdout=subprocess.DEVNULL,
        )
        subprocess.check_call(
            [get_docker_path(), "rm", self.container_id], stdout=subprocess.DEVNULL
        )
    ...
    def command_as_list(self, commands_to_run):
        ...
        return [
            ...
            "{} {} '{}'".format(
                os.path.join(get_tools_helper_dir(), "run_in_umake_dir"),
                self.UMAKE_TOOLS_IN_CONTAINER,
                commands_to_run,
            ),
        ]
    ...
    def check_and_kill_process(self, process_grep, wait_before=0, send_sigkill=False):
        ...
        if not self._exec_command(
            self.command_as_list(
                "{} {} {}".format(
                    os.path.join(get_tools_helper_dir(), "check_and_kill_process"),
                    send_sigkill,
                    " ".join(process_grep),
                )
            )
            ...
            raise BaseException(
                "The process we try to find and kill can't be found: {}".format(
                    process_grep
                )
            )
    ...
    def _get_path_from_desktop_file(self, key, abspath_transform=None):
        ...
        command = self.command_as_list(
            [
                os.path.join(get_tools_helper_dir(), "get_path_from_desktop_file"),
    ...
    def get_launcher_path(self, desktop_filename):
        ...
        command = self.command_as_list(
            [
                os.path.join(get_tools_helper_dir(), "get_launcher_path"),
                desktop_filename,
            ]
        )
    ...
    def launcher_exists_and_is_pinned(self, desktop_filename):
        ...
        command = self.command_as_list(
            [
                os.path.join(
                    get_tools_helper_dir(), "check_launcher_exists_and_is_pinned"
                ),
                desktop_filename,
            ]
        )
    ...
    def path_exists(self, path):
        ...
        command = self.command_as_list(
            [os.path.join(get_tools_helper_dir(), "path_exists"), path]
        )
        return self._exec_command(command)[0]
    ...
    def remove_path(self, path):
        ...
        path = path.replace(os.environ["HOME"], "/home/{}".format(self.DOCKER_USER))
        command = self.command_as_list(
            [os.path.join(get_tools_helper_dir(), "remove_path"), path]
        )
    ...
    def is_in_path(self, filename):
        ...
        return self._exec_command(
            self.command_as_list(["bash", "-l", "which", filename])
        )[0]

    def is_in_group(self, group):
        """Check inside the container if the user is in a group"""
        command = self.command_as_list(
            [os.path.join(get_tools_helper_dir(), "check_user_in_group"), group]
        )
        return self._exec_command(command)[0]
    ...
    def get_file_perms(self, path):
        ...
        command = self.command_as_list(
            [os.path.join(get_tools_helper_dir(), "get_file_perms"), path]
        )
        success, stdout, stderr = self._exec_command(command)
        if success:
            return stdout
