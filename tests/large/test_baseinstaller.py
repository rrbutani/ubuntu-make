...
class BaseInstallerTests(LargeFrameworkTests):
    ...
    def setUpClass(cls):
        ...
        cls.download_page_file_path = os.path.join(
            get_data_dir(), "server-content", "localhost", "index.html"
        )
        if not cls.in_container:
            ...
            framework_dir = os.path.expanduser(
                os.path.join("~", ".umake", "frameworks")
            )
            cls.testframework = os.path.join(framework_dir, "baseinstallerfake.py")
            os.makedirs(framework_dir, exist_ok=True)
            shutil.copy(
                os.path.join(get_data_dir(), "testframeworks", "baseinstallerfake.py"),
                cls.testframework,
            )
    ...
    def setUp(self):
        ...
        self.installed_path = os.path.join(
            self.install_base_path, "base", "base-framework"
        )
    ...
    def test_default_install(self):
        ...
        self.check_and_kill_process(
            [self.JAVAEXEC, self.installed_path], wait_before=self.TIMEOUT_START
        )
    ...
    def test_doesnt_accept_wrong_path(self):
        ...
        self.child.sendline(chr(127) * 100 + "/")
        self.expect_and_no_warn(
            "Choose installation path: {}".format(self.installed_path), expect_warn=True
        )
    ...
    def test_reinstall(self):
        ...
        for loop in ("install", "reinstall"):
            self.child = spawn_process(
                self.command("{} base base-framework".format(UMAKE))
            )
            if loop == "reinstall":
    ...
    def test_reinstall_other_path(self):
        ...
        for loop in ("install", "reinstall"):
            if loop == "reinstall":
                ...
                self.child = spawn_process(
                    self.command(
                        "{} base base-framework {}".format(UMAKE, self.installed_path)
                    )
                )
                self.expect_and_no_warn("Base Framework is already installed.*\[.*\] ")
                self.child.sendline("y")
    ...
    def test_reinstall_other_non_empty_path(self):
        ...
        for loop in ("install", "reinstall"):
            if loop == "reinstall":
                ...
                self.child.sendline("y")
                self.expect_and_no_warn(
                    "{} isn't an empty directory.*there\? \[.*\] ".format(
                        self.installed_path
                    )
                )
                self.child.sendline("y")
    ...
    def test_custom_install_path(self):
        ...
        self.installed_path = "/tmp/foo"
        self.child = spawn_process(
            self.command("{} base base-framework {}".format(UMAKE, self.installed_path))
        )
        self.expect_and_no_warn(
            "\[I Accept.*\]"
        )  # ensure we have a license as the first question
        self.child.sendline("a")
        self.expect_and_no_warn(
            "Installation done", timeout=self.TIMEOUT_INSTALL_PROGRESS
        )
    ...
    def test_start_install_on_existing_dir_refuse(self):
        ...
        self.expect_and_no_warn(
            "{} isn't an empty directory.*there\? \[.*\] ".format(self.installed_path)
        )
        self.accept_default_and_wait()
        self.close_and_check_status()
    ...
    def test_is_default_framework_with_user_path(self):
        ...
        self.installed_path = "/tmp/foo"
        self.child = spawn_process(
            self.command("{} base {}".format(UMAKE, self.installed_path))
        )
        self.expect_and_no_warn(
            "\[I Accept.*\]"
        )  # ensure we have a license as the first question
    ...
    def test_removal(self):
        ...
        # now, remove it
        self.child = spawn_process(
            self.command("{} base base-framework --remove".format(UMAKE))
        )
        self.wait_and_close()
    ...
    def test_removal_global_option(self):
        ...
        # now, remove it
        self.child = spawn_process(
            self.command("{} --remove base base-framework".format(UMAKE))
        )
        self.wait_and_close()
    ...
    def test_automated_install(self):
        ...
        self.child = spawn_process(
            self.command(
                "{} base base-framework {} --accept-license".format(
                    UMAKE, self.installed_path
                )
            )
        )
        self.expect_and_no_warn(
    ...
    def test_install_with_wrong_md5sum(self):
        ...
        with swap_file_and_restore(self.download_page_file_path) as content:
            ...
            self.expect_and_no_warn(
                "\[I Accept.*\]"
            )  # ensure we have a license question
            self.child.sendline("a")
            self.expect_and_no_warn(
                [pexpect.EOF, "Corrupted download? Aborting."],
    ...
    def test_install_with_no_download_links(self):
        ...
        with swap_file_and_restore(self.download_page_file_path) as content:
            ...
            self.child.sendline("")
            self.expect_and_no_warn(
                [pexpect.EOF], timeout=self.TIMEOUT_INSTALL_PROGRESS, expect_warn=True
            )
            self.wait_and_close(exit_status=1)
    ...
    def test_install_with_404(self):
        ...
        with swap_file_and_restore(self.download_page_file_path) as content:
            ...
            self.child.sendline("a")
            self.expect_and_no_warn(
                [pexpect.EOF, "ERROR: 404 Client Error: File not found"],
                timeout=self.TIMEOUT_INSTALL_PROGRESS,
                expect_warn=True,
            )
            self.wait_and_close(exit_status=1)
