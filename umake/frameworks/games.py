...
class GamesCategory(umake.frameworks.BaseCategory):
    def __init__(self):
        super().__init__(
            name="Games", description=_("Games Development Environment"), logo_path=None
        )
...
class Stencyl(umake.frameworks.baseinstaller.BaseInstaller):
    def __init__(self, **kwargs):
        super().__init__(
            name="Stencyl",
            description=_("Stencyl game developer IDE"),
...
class Blender(umake.frameworks.baseinstaller.BaseInstaller):
    def __init__(self, **kwargs):
        super().__init__(
            name="Blender",
            description=_("Very fast and versatile 3D modeller/renderer"),
            only_on_archs=["i386", "amd64"],
    ...
    arch_trans = {"amd64": "x86_64", "i386": "i686"}
    ...
    def parse_download_link(self, line, in_download):
        ...
        if ".tar.bz2" in line:
            p = re.search(
                r'href=\"(https://www\.blender\.org/[^<]*{}\.tar\.bz2)/?"'.format(
            ...
            with suppress(AttributeError):
                ...
                filename = (
                    "release"
                    + re.search("blender-(.*)-linux", url).group(1).replace(".", "")
                    + ".md5"
                )
                self.checksum_url = (
                    os.path.join(os.path.dirname(url), filename)
                    .replace("download", "release")
                    .replace("www", "download")
                )
                url = url.replace(
                    "www.blender.org/download", "download.blender.org/release"
                )
        return ((url, None), in_download)
    ...
    def post_install(self):
        ...
        create_launcher(
            ...
            get_application_desktop_file(
                name=_("Blender"),
                icon_path=os.path.join(
                    self.install_path, "icons", "scalable", "apps", "blender.svg"
                ),
                try_exec=self.exec_path,
                exec=self.exec_link_name,
                comment=self.description,
...
def _chrome_sandbox_setuid(path):
    ...
    with as_root():
        try:
            ...
            logger.error(
                "Couldn't change owner and file perm to {}: {}".format(path, e)
            )
            return False
...
class Unity3D(umake.frameworks.baseinstaller.BaseInstaller):
    def __init__(self, **kwargs):
        super().__init__(
            name="Unity3d",
            description=_("Unity 3D Editor Linux experimental support"),
            only_on_archs=["amd64"],
    ...
    def parse_download_link(self, line, in_download):
        ...
        if in_download:
            p = re.search(
                r'a href="(https:\/\/beta\.unity3d\.com\/download\/[^\/]+)', line
            )
            with suppress(AttributeError):
    ...
    def post_install(self):
        ...
        with futures.ProcessPoolExecutor(max_workers=1) as executor:
            ...
            f = executor.submit(
                _chrome_sandbox_setuid,
                os.path.join(self.install_path, "chrome-sandbox"),
            )
            if not f.result():
        ...
        create_launcher(
            self.desktop_filename,
            get_application_desktop_file(
                name=_("Unity3D Editor"),
                icon_path=os.path.join(
                    self.install_path, "Data", "Resources", "LargeUnityIcon.png"
                ),
                try_exec=self.exec_path,
                exec=self.exec_link_name,
                comment=self.description,
...
class Twine(umake.frameworks.baseinstaller.BaseInstaller):
    def __init__(self, **kwargs):
        super().__init__(
            name="Twine",
            description=_("Twine tool for creating interactive and nonlinear stories"),
            only_on_archs=["i386", "amd64"],
...
class Superpowers(umake.frameworks.baseinstaller.BaseInstaller):
    def __init__(self, **kwargs):
        super().__init__(
            name="Superpowers",
            description=_("The HTML5 2D+3D game maker"),
            only_on_archs=["i386", "amd64"],
    ...
    arch_trans = {"amd64": "x64", "i386": "ia32"}
...
class GDevelop(umake.frameworks.baseinstaller.BaseInstaller):
    def __init__(self, **kwargs):
        super().__init__(
            name="GDevelop",
            description=_("Create your own games"),
            only_on_archs=["i386", "amd64"],
...
class Godot(umake.frameworks.baseinstaller.BaseInstaller):
    def __init__(self, **kwargs):
        super().__init__(
            name="Godot",
            description=_("The game engine you waited for"),
            only_on_archs=["i386", "amd64"],
        ...
        self.icon_url = (
            "https://godotengine.org/themes/godotengine/assets/download/godot_logo.svg"
        )
