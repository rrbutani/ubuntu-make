...
class WebCategory(umake.frameworks.BaseCategory):
    def __init__(self):
        super().__init__(
            name="Web", description=_("Web Developer Environment"), logo_path=None
        )
...
class FirefoxDev(umake.frameworks.baseinstaller.BaseInstaller):
    def __init__(self, **kwargs):
        super().__init__(
            name="Firefox Dev",
            description=_("Firefox Developer Edition"),
    ...
    def get_metadata_and_check_license(self, result):
        ...
        reg_expression = r'href="(\S+firefox-devedition\S+os=linux{}&amp;lang=\S+)"'.format(
            tag_machine
        )
        languages = []
        ...
        if self.arg_lang:
            ...
            UI.delayed_display(
                TextWithChoices(
                    _("Choose language: {}".format(default_label)), languages, True
                )
            )
...
class PhantomJS(umake.frameworks.baseinstaller.BaseInstaller):
    def __init__(self, **kwargs):
        super().__init__(
            name="PhantomJS",
            description=_("headless WebKit scriptable with a JavaScript API"),
...
class Geckodriver(umake.frameworks.baseinstaller.BaseInstaller):
    ...
    arch_trans = {"amd64": "linux32", "i386": "linux64", "armhf": "arm7hf"}

    def parse_download_link(self, line, in_download):
        url = None
        for asset in line["assets"]:
            if (
                "{}.tar.gz".format(self.arch_trans[get_current_arch()])
...
class Chromedriver(umake.frameworks.baseinstaller.BaseInstaller):
    def __init__(self, **kwargs):
        super().__init__(
            name="Chromedriver",
            description=_("WebDriver for Chrome"),
            only_on_archs=["amd64"],
