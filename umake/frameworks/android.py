...
class AndroidStudio(umake.frameworks.baseinstaller.BaseInstaller):
    def __init__(self, **kwargs):
        super().__init__(
            name="Android Studio",
            description=_("Android Studio (default)"),
            is_category_default=True,
            only_on_archs=_supported_archs,
            expect_license=True,
            packages_requirements=[
                "openjdk-7-jdk | openjdk-8-jdk | openjdk-11-jdk",
                "libc6:i386",
                "libncurses5:i386",
                "libstdc++6:i386",
                "lib32z1",
    ...
    def parse_license(self, line, license_txt, in_license):
        ...
        return self.category.parse_license(
            '<div id="studio_linux_bundle_download"', line, license_txt, in_license
        )

    def parse_download_link(self, line, in_download):
        """Parse Android Studio download link, expect to find a sha1sum and a url"""
        return self.category.parse_download_link(
    ...
    def post_install(self):
        ...
        add_env_to_user(
            self.name,
            {
                "ANDROID_HOME": {"value": self.install_path, "keep": False},
...
class AndroidSDK(umake.frameworks.baseinstaller.BaseInstaller):
    ...
    def parse_license(self, line, license_txt, in_license):
        ...
        return self.category.parse_license(
            '<div id="sdk_linux_download"', line, license_txt, in_license
        )
...
class AndroidPlatformTools(umake.frameworks.baseinstaller.BaseInstaller):
    def __init__(self, **kwargs):
        super().__init__(
            ...
            description=_("Android Platform Tools"),
            only_on_archs=_supported_archs,
            expect_license=True,
    ...
    def post_install(self):
        ...
        add_env_to_user(
            self.name,
            {"PATH": {"value": [os.path.join(self.install_path, "platform-tools")]}},
        )
        UI.delayed_display(DisplayMessage(self.RELOGIN_REQUIRE_MSG.format(self.name)))
...
class AndroidNDK(umake.frameworks.baseinstaller.BaseInstaller):
    ...
    def parse_license(self, line, license_txt, in_license):
        ...
        return self.category.parse_license(
            '<div id="ndk_linux64_download"', line, license_txt, in_license
        )

    def parse_download_link(self, line, in_download):
        """Parse Android NDK download link, expect to find a sha1sum and a url"""
        return self.category.parse_download_link(
            "ndk_linux64_download",
            line,
            in_download,
            r"href=\"(https://dl.google.com.*-linux.*.zip)\"",
        )
    ...
    def post_install(self):
        ...
        add_env_to_user(
            self.name,
            {
                "NDK_ROOT": {"value": self.install_path, "keep": False},
        ...
        UI.display(
            DisplayMessage(
                "NDK installed in {}. More information on how to use it on {}".format(
                    self.install_path,
...
class EclipseADTForRemoval(umake.frameworks.baseinstaller.BaseInstaller):
    def __init__(self, **kwargs):
        super().__init__(
            name="Eclipse ADT",
            description="For removal only (not supported upstream anymore)",
            download_page=None,
            only_on_archs=_supported_archs,
