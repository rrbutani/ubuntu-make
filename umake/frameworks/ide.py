...
class IdeCategory(umake.frameworks.BaseCategory):
    def __init__(self):
        super().__init__(name="IDE", description=_("Generic IDEs"), logo_path=None)
...
class BaseEclipse(umake.frameworks.baseinstaller.BaseInstaller, metaclass=ABCMeta):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.icon_url = os.path.join(
            "https://www.eclipse.org/downloads/", "images", self.icon_filename
        )
    ...
    def download_provider_page(self):
        ...
        DownloadCenter(
            [DownloadItem(self.download_page, headers=self.headers)],
        ...
        if in_download:
            ...
            with suppress(AttributeError):
                self.new_download_url = (
                    "https://www.eclipse.org" + p.group(2) + ".sha512&r=1"
                )
    ...
    def post_install(self):
        ...
        DownloadCenter(
            urls=[DownloadItem(self.icon_url, None)],
...
class BaseJetBrains(umake.frameworks.baseinstaller.BaseInstaller, metaclass=ABCMeta):
    ...
    def __init__(self, *args, **kwargs):
        ...
        if self.executable:
            ...
            kwargs["required_files_path"] = current_required_files_path
        download_page = "https://data.services.jetbrains.com/products/releases?code={}".format(
    ...
    def install_framework_parser(self, parser):
        ...
        this_framework_parser.add_argument(
            "--eap", action="store_true", help=_("Install EAP version if available")
        )
        ...
        if in_download:
            ...
            with suppress(AttributeError):
                self.new_download_url = self.download_page.replace(
                    ...
                    + "{}-netbeans-{}-bin.zip.sha512".format(p.group(1), p.group(2)),
                )
        return (None, in_download)
...
class VisualStudioCode(umake.frameworks.baseinstaller.BaseInstaller):
    ...
    def __init__(self, **kwargs):
        super().__init__(
            ...
            description=_("Visual Studio focused on modern web and cloud"),
            only_on_archs=["i386", "amd64"],
            expect_license=True,
...
class LightTable(umake.frameworks.baseinstaller.BaseInstaller):
    def __init__(self, **kwargs):
        super().__init__(
            name="LightTable",
            description=_("LightTable code editor"),
            only_on_archs=["amd64"],
    ...
    def post_install(self):
        ...
        create_launcher(
            ...
            get_application_desktop_file(
                name=_("LightTable"),
                icon_path=os.path.join(
                    self.install_path, "resources", "app", "core", "img", "lticon.png"
                ),
                try_exec=self.exec_path,
                exec=self.exec_link_name,
...
class Atom(umake.frameworks.baseinstaller.BaseInstaller):
    def __init__(self, **kwargs):
        super().__init__(
            name="Atom",
            description=_("The hackable text editor"),
            only_on_archs=["amd64"],
    ...
    def post_install(self):
        ...
        # Add apm to PATH
        add_exec_link(
            os.path.join(self.install_path, "resources", "app", "apm", "bin", "apm"),
    ...
    def install_framework_parser(self, parser):
        this_framework_parser = super().install_framework_parser(parser)
        this_framework_parser.add_argument(
            "--beta", action="store_true", help=_("Install Beta version if available")
        )
        return this_framework_parser
...
class DBeaver(umake.frameworks.baseinstaller.BaseInstaller):
    def __init__(self, **kwargs):
        super().__init__(
            name="DBeaver",
            description=_("Free universal database manager and SQL client"),
    ...
    arch_trans = {"amd64": "x86_64", "i386": "x86"}

    def parse_download_link(self, line, in_download):
        url = None
        for asset in line["assets"]:
            if (
                "linux.gtk.{}.tar.gz".format(self.arch_trans[get_current_arch()])
...
class SublimeText(umake.frameworks.baseinstaller.BaseInstaller):
    def __init__(self, **kwargs):
        super().__init__(
            name="Sublime Text",
            description=_("Sophisticated text editor for code, markup and prose"),
            only_on_archs=["i386", "amd64"],
    ...
    def parse_download_link(self, line, in_download):
        ...
        if ".tar.bz2" in line:
            p = re.search(
                r'href="([^<]*{}.tar.bz2)"'.format(self.arch_trans[get_current_arch()]),
                line,
            )
            with suppress(AttributeError):
                url = p.group(1)
    ...
    def post_install(self):
        ...
        create_launcher(
            ...
            get_application_desktop_file(
                name=_("Sublime Text"),
                icon_path=os.path.join(
                    self.install_path, "Icon", "128x128", "sublime-text.png"
                ),
                try_exec=self.exec_path,
                exec=self.exec_link_name,
...
class Processing(umake.frameworks.baseinstaller.BaseInstaller):
    def __init__(self, **kwargs):
        super().__init__(
            name="Processing",
            description=_("Processing code editor"),
            only_on_archs=["i386", "amd64"],
    ...
    arch_trans = {"amd64": "64", "i386": "32"}

    def parse_download_link(self, line, in_download):
        url = None
        for asset in line["assets"]:
            if (
                "linux{}".format(self.arch_trans[get_current_arch()])
                in asset["browser_download_url"]
            ):
                in_download = True
                url = asset["browser_download_url"]
    ...
    def post_install(self):
        ...
        create_launcher(
            ...
            get_application_desktop_file(
                name=_("Processing"),
                icon_path=os.path.join(
                    self.install_path, "lib", "icons", "pde-256.png"
                ),
                try_exec=self.exec_path,
                exec=self.exec_link_name,
...
class LiteIDE(umake.frameworks.baseinstaller.BaseInstaller):
    def __init__(self, **kwargs):
        super().__init__(
            name="LiteIDE",
            description=_("LiteIDE is a simple, open source, cross-platform Go IDE."),
            only_on_archs=["i386", "amd64"],
    ...
    def parse_download_link(self, line, in_download):
        ...
        for asset in line["assets"]:
            if (
                "linux{}-qt5".format(self.arch_trans[get_current_arch()])
                in asset["browser_download_url"]
            ):
                in_download = True
                url = asset["browser_download_url"]
...
class RStudio(umake.frameworks.baseinstaller.BaseInstaller):
    def __init__(self, **kwargs):
        super().__init__(
            name="RStudio",
            description=_("RStudio code editor"),
            only_on_archs=["amd64"],
    ...
    def parse_download_link(self, line, in_download):
        ...
        checksum = None
        if (
            get_current_distro_version().split(".")[0] < "18"
            or get_current_distro_version(name="debian") < "9"
        ):
        ...
        if "-debian.tar.gz" in line:
            p = re.search(
                r"href=\"([^<]*{}.*-debian\.tar\.gz)\"".format(ubuntu_version), line
            )
            with suppress(AttributeError):
                url = p.group(1)
                in_download = True
...
class Arduino(Arduino):
    def setup(self, *args, **kwargs):
        ...
        logger.warning(
            "Arduino is now in the electronics category, please refer it from this category from now on. "
